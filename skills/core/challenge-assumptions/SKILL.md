...
- Disproof signals are generated by negating the assumption and extracting key phrases (e.g., turning "X happens in Y" into "not in Y", "X does not happen in Y", "Z handles X").
- Matching is case-insensitive and punctuation-insensitive.
- A match is reported when a disproof signal appears within a configurable context window of the assumption text in the evidence.
