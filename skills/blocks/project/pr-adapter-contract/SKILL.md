...
Building block that defines the normalized interface between conductor skills and their adapters. Adapters translate provider-specific data into these shapes; the conductor owns synthesis and reporting.

## Skill type

Building block. Reusable reference for adapter authors and conductors.

## When to use

- Authoring a new adapter for a PR platform, CI system, static-analysis tool, issue tracker, notification source, or context-report source.
- Reviewing an adapter to ensure it follows the contract.
- Implementing a conductor that consumes adapter data.
- Mapping a new source into the normalized shape.

## In scope

- Define the normalized adapter envelope and its status values.
...
- Fixing code or replying to comments.
...
See [references/INTERFACE.md](references/INTERFACE.md) for the full operation schemas.
...
- `token-resolver` — token resolution order
