...
class CheckContextFreshnessTests(unittest.TestCase):
    def test_fresh_report(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            data = json.loads(result.stdout)
            self.assertTrue(data["fresh"], data)

    def test_stale_commit_is_not_fresh(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            report = Path(tmpdir) / "report.md"
            report.write_text(
                "---\n"
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            data = json.loads(result.stdout)
            self.assertFalse(data["fresh"], data)
            self.assertIn("Commit mismatch", data["reason"])
