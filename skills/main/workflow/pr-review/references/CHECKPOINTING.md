...
5. If context was compacted, re-read all evidence from `evidence-store` for the work item.

## Evidence-first design

`pr-review` stores each capability's tool output in `evidence-store` under the identity `{work_item_type}/{work_item_key}`. Reports are views over that evidence. When resuming, query the evidence store instead of re-invoking tools if the evidence is fresh.
