...
Classify the user's initial request into one of the `write-a-skill` top-level branches: `initialize`, `explore`, `create`, or `change`. If the intent is ambiguous, return `needs_input` with a single clarifying question and a proposed default.
...
### Out of scope
- Choosing the internal gate (`full`, `quick`, `review`, `update`). That is a follow-up question handled by the conductor.
- Designing the skill.
- Auditing the skill.
- Writing files.
